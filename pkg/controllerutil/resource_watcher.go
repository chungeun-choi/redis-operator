...
func (w *ResourceWatcher) handleEvent(meta metav1.Object, queue workqueue.RateLimitingInterface) {
	...
	w.RLock()
	deps, ok := w.watched[changedObjectName]
	if !ok {
		w.RUnlock()
		return
	}
	// Copy slice under RLock to safely release lock before queueing
	depCopy := make([]types.NamespacedName, len(deps))
	copy(depCopy, deps)
	w.RUnlock()

	// Enqueue reconciliation for each dependent object.
	for _, dep := range depCopy {
